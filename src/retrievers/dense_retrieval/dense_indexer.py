...
# Paths
ROOT = Path(__file__).resolve().parents[3]  # repo root (go up three levels from dense_retrieval/)
CORPUS = ROOT / "data" / "corpus" / "corpus_subsection_v1.jsonl"
OUTDIR = ROOT / "artefacts" / "dense_retriever" 
...
def build_index(corpus_path: Path, model_path: str) -> Tuple[Dict, Dict, faiss.Index]:
    texts, chunk_ids = [], []
...
def save_faiss_bundle(out_dir: Path,
                      index: faiss.Index,
                      chunk_ids: np.ndarray,
                      meta: Dict[str, Any]):
    out_dir.mkdir(parents=True, exist_ok=True)
    faiss.write_index(index, str(out_dir / "index.faiss"))
    np.save(out_dir / "chunk_ids.npy", chunk_ids)
    with open(out_dir / "meta.json", "w") as f:
        json.dump(meta, f, indent=2)
    print(f"Saved: {out_dir/'index.faiss'}, {out_dir/'chunk_ids.npy'}, {out_dir/'meta.json'}")

def load_faiss_bundle(in_dir: Path) -> Tuple[faiss.Index, np.ndarray, Dict[str, Any]]:
    index = faiss.read_index(str(in_dir / "index.faiss"))
    chunk_ids = np.load(in_dir / "chunk_ids.npy", allow_pickle=True)
    with open(in_dir / "meta.json") as f:
        meta = json.load(f)
    return index, chunk_ids, meta
...
def main():
    ...
    # Save artifacts
    np.savez_compressed(OUTDIR / "faiss" / "embeddings.npz", **artifacts["dense"])
    (OUTDIR / "faiss" / "meta.json").write_text(
    ...
    # Save FAISS index
    faiss_index_path = OUTDIR / "faiss" / "index.faiss"
