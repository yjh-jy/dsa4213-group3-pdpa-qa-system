...
# --- Paths ---
ROOT = Path(__file__).resolve().parents[3]      # repo root (go up three levels from hybrid_retrieval/)
...
RRF_K = 60  # optimized RRF constant
...
def load_dense_retriever(checkpoint_path=None):
    ...
    """
    import sys
    sys.path.append(str(ROOT / "artefacts" / "dense_retriever" / "model"))
    from dense_retriever import DenseRetriever
    
    # Initialize dense retriever with optional checkpoint
    if checkpoint_path:
        dense_retriever = DenseRetriever(model_name=checkpoint_path)
    else:
...
class HybridRetriever:
    ...
    def retrieve_dense(self, query: str, top_k=20) -> Dict[str, float]:
        ...
        result = self.dense_retriever.faiss_search(query, top_k=top_k)
    ...
    def search(self, query: str, top_k=10):
        ...
        for rank, (chunk_id, score, section_id) in enumerate(results, 1):
            ...
            chunk_info = self.sections_map.get(chunk_id, {})
