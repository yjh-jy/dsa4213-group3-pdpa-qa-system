...
sys.path.append(str(ROOT / "hybrid_retrieval"))
sys.path.append(str(ROOT / "retrieval_evaluation"))
...
class PDPAPipeline:
    def __init__(self):
        ...
        (self.artifacts_dir / "dense_retriever").mkdir(exist_ok=True)
        (self.artifacts_dir / "faiss").mkdir(exist_ok=True)
        (self.artifacts_dir / "tuning").mkdir(exist_ok=True)
        (self.artifacts_dir / "eval").mkdir(exist_ok=True)
    ...
    def _ensure_bm25_index(self):
        ...
        bm25_index_dir = self.root / "bm25_retrieval" / "indexer_results" / "pdpa_v1"
    ...
    def _validate_data(self):
        ...
        required_files = [
            ...
            self.training_data_dir / "corpus.jsonl",
    ...
    def _cleanup_artifacts(self):
        ...
        cleanup_dirs = [
            ("Dense Retriever Model", self.artifacts_dir / "dense_retriever"),
            ("FAISS Index", self.artifacts_dir / "faiss"),
            ("Tuning Results", self.artifacts_dir / "tuning"),
            ("Evaluation Results", self.artifacts_dir / "eval")
        ]
        
        cleanup_files = [
            ("Final Results", self.artifacts_dir / "results.json")
        ]
    ...
    def train_dense_retriever(self) -> str:
        ...
        corpus_path = self.training_data_dir / "corpus.jsonl"
        ...
        faiss_dir = self.artifacts_dir / "faiss"
        
        # Reload the fine-tuned model for FAISS index building
        print(f"Loading model for FAISS indexing: {model_output_dir}")
        fine_tuned_retriever = DenseRetriever(model_name=str(model_output_dir))
        print(f"FAISS indexing model using device: {fine_tuned_retriever.device}")
        
        # Confirm model type for FAISS indexing
        if hasattr(fine_tuned_retriever, 'is_fine_tuned'):
            if fine_tuned_retriever.is_fine_tuned:
                print(f"✓ Fine-tuned model loaded for FAISS indexing")
            else:
                print(f"⚠️ Warning: Expected fine-tuned model but loaded base model for FAISS indexing")
        
        print(f"✓ GPU acceleration confirmed for FAISS index building")
        
        # Save embeddings for FAISS (embeddings are already computed in __init__)
        import numpy as np
        np.savez_compressed(
            faiss_dir / "embeddings.npz",
            embeddings=fine_tuned_retriever.corpus_embeddings,
            chunk_ids=fine_tuned_retriever.chunk_ids
        )
        
        # Save sections mapping
        with open(faiss_dir / "sections_map.json", "w") as f:
            json.dump(fine_tuned_retriever.sections_map, f, indent=2)
    ...
    def tune_hybrid_retriever(self, dense_model_path: str) -> Dict:
        ...
        # Save tuning results
        tuning_file = self.artifacts_dir / "tuning" / "hyperparameter_tuning.json"
    ...
    def run_pipeline(self):
        ...
        # Clean up all artifacts to ensure fresh run
        ...
        try:
            ...
            self.results["dense_retriever"] = {
                "model_dir": str(self.artifacts_dir / "dense_retriever"),
                "faiss_index_dir": str(self.artifacts_dir / "faiss"),
            ...
            results_file = self.artifacts_dir / "results.json"
